...
- **`zukiru::shaderc`** — a small library: `compileGlslToSpirv(source, stage)` →
  `Result<std::vector<u32>>`.
- **`zukiru-shaderc`** — a CLI front-end that turns a `.vert`/`.frag`/… file into
  a `.spv` binary.
...
```bash
cmake --preset debug -DZUKIRU_BUILD_TOOLS=ON
cmake --build build/debug --target zukiru-shaderc
```

## CLI

```bash
zukiru-shaderc triangle.vert -o triangle.vert.spv        # stage from extension
zukiru-shaderc shader.glsl   -o out.spv --stage fragment # or explicit
```
...
```bash
ctest --preset debug -R '^shaderc\.'   # requires -DZUKIRU_BUILD_TOOLS=ON
```
