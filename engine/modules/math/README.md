...
Namespace: `zukiru::math` (nested, per the standard convention — `core` is the only
...
| Header | Contents |
|--------|----------|
| `<zukiru/math/scalar.hpp>` | Constants (`kPi`, `kDegToRad`, `kEpsilon`, …), `radians`/`degrees`, `lerp`, `clamp`, `saturate`, `sign`, `approxEqual`/`approxZero`, generic `min`/`max`. |
| `<zukiru/math/vec.hpp>` | `Vec2`/`Vec3`/`Vec4`: operators, `dot`, `cross`, `length`, `normalize`, `distance`, `reflect`, `lerp`, component-wise min/max. |
| `<zukiru/math/mat.hpp>` | `Mat3`/`Mat4`: multiply, `transpose`, `determinant`, `inverse`, `translation`/`scale`, point/direction transforms. |
| `<zukiru/math/quat.hpp>` | `Quat`: `fromAxisAngle`/`fromEuler`, compose, `rotate`, `conjugate`/`inverse`, `slerp`, `toMat3`/`toMat4`. |
| `<zukiru/math/transform.hpp>` | `Transform` (TRS), `lookAt`, `perspective`, `orthographic`. |
| `<zukiru/math/geometry.hpp>` | `Aabb`, `Ray`, `Plane`, `Sphere` + `raySphere`/`rayPlane`/`rayAabb`. |
| `<zukiru/math/math.hpp>` | Umbrella header. |
